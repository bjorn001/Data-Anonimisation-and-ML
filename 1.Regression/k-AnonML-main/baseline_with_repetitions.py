...
from utils.types import AnonMethod, Classifier, Dataset, MLRes, MLRes2
...
def main(args):
    ...
    output_path2 = os.path.join(res_folder, f'{dataset}_{os.path.basename(anon_method)}_{os.path.basename(classifier)}_k_{args.stop_k}_MSE.csv')
    ...
    if dataset == Dataset.CMC:
        ...
        IS_CAT2 = [False, False, False, False]
        max_numeric = {"latitude": 119.33, "longitude": 37.245, "housing_median_age": 32.5, "median_income": 5.2035, }
        xgb_eval_metric = 'merror'
    ...
    if dataset == Dataset.ADULT:
        ...
        SA_var = ['ID', 'median_house_value']#,'mean_rooms', 'mean_bedrooms','population','households', 'ocean_proximity']
    ...
    print("PRINTING NON-ANONYMISED SCORE NOW:")
    print(clf.score(X_test,y_test))

    #print(data.isnull().sum())
    #print("PRINTING MSE NOW:")
        ...
        for s in s_range:
            ...
            for k in k_range:
                ...
                for node in range(nodes_count):
                    ...
                    X = pd.get_dummies(X, columns=one_hot_original, drop_first=True)
                    scaler = MinMaxScaler()
                    scaler.fit(X)
                    X = scaler.transform(X)
                   ...
                   #print(set(np.asarray(y_test)))
                    ...
                    # append example variance, Absolute MSE, MSE, r2 score to the ML results
                    print("PRINTING score OUTPUT NOW:")
                    print(clf.score(X_test,y_test))
                    ...
                    #print("Success!")
                    # MSE from cb onwards
                    #for i, res in enumerate(util.get_regression_metrics(np.asarray(y_test), pred_test)):
                    
                    ml_res.append(clf.score(X_test,y_test))
                    print("PRINTING ml_res")
                    print(ml_res)       
                    ml_res2.append(sqrt(mean_squared_error(y_test,pred_test)))

                    #print("WRITING RESULTS NOW:")
                ...
                if args.debug:
                    ...
                    util.write_results(s, ml_res2, args.anon_method, output_path2, num=i)
                    ml_res = MLRes()
                    ml_res2 = MLRes2()

            util.write_results(s, ml_res, args.anon_method, output_path)
            util.write_results(s, ml_res2, args.anon_method, output_path2)         
